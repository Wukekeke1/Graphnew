...
from model import PointNet, PointNet_norm, DGCNN, DGCNN_norm, DGCNN_geo, DGCNN_norm_geo, HGCNN, HGCNN_norm, HGCNN_geo, HGCNN_norm_geo,HGCNN_cov,Graphcnn,Graphcnn_norm,Graphcnn_cov,Graphcnn_norm_geo,Graphcnn_geo
import numpy as np
...
def train(args, io):
    ...
    if args.model == 'pointnet':
        ...
        test_loader = DataLoader(DefectsDataset_HGCNnorm_geo(partition='test', 
                                                             ...
                                                             root = args.dataset,data_augmentation=False),
                                 num_workers=8,batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    elif args.model == 'hgcnn_cov':
        model = HGCNN_cov(args).to(device)
        train_loader = DataLoader(DefectsDataset_HGCN(partition='train', 
                                                  num_points=args.num_points, 
                                                  root = args.dataset, data_augmentation=True),
                                  num_workers=8,batch_size=args.batch_size, shuffle=True, drop_last=True)
        ...
        test_loader = DataLoader(DefectsDataset_HGCN(partition='test', 
                                                     ...
                                                     root = args.dataset,data_augmentation=False), 
                                 num_workers=8,batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    elif args.model == 'graphcnn_norm':
        model = Graphcnn_norm(args).to(device)
        train_loader = DataLoader(DefectsDataset_HGCNnorm(partition='train', 
                                                          num_points=args.num_points, 
                                                          root = args.dataset, data_augmentation=True),
                                  num_workers=8,batch_size=args.batch_size, shuffle=True, drop_last=True)
    ...
    elif args.model == 'graphcnn_cov':
        model = Graphcnn_cov(args).to(device)
        train_loader = DataLoader(DefectsDataset_HGCN(partition='train', 
                                                  num_points=args.num_points, 
                                                  root = args.dataset, data_augmentation=True),
                                  num_workers=8,batch_size=args.batch_size, shuffle=True, drop_last=True)
        test_loader = DataLoader(DefectsDataset_HGCN(partition='test', 
                                                     ...
                                                     root = args.dataset,data_augmentation=False), 
                                 num_workers=8,batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    elif args.model == 'graphcnn_norm_geo':
        model = Graphcnn_norm_geo(args).to(device)
        train_loader = DataLoader(DefectsDataset_HGCNnorm_geo(partition='train', 
                                                              num_points=args.num_points, 
                                                              root = args.dataset, data_augmentation=True),
                                  num_workers=8,batch_size=args.batch_size, shuffle=True, drop_last=True)
        test_loader = DataLoader(DefectsDataset_HGCNnorm_geo(partition='test', 
                                                             ...
                                                             root = args.dataset,data_augmentation=False),
                                 num_workers=8,batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    elif args.model == 'graphcnn_geo':
        model = Graphcnn_geo(args).to(device)
        train_loader = DataLoader(DefectsDataset_HGCN_geo(partition='train', 
                                                              num_points=args.num_points, 
                                                              root = args.dataset, data_augmentation=True),
                                  num_workers=8,batch_size=args.batch_size, shuffle=True, drop_last=True)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        std_per_class_acc = np.std(per_class_acc)
        outstr = 'Test %d, loss: %.6f, test acc: %.6f, test avg acc: %.6f, test std:%.6f' % (epoch,
                                                                              test_loss*1.0/count,
                                                                              test_acc,
                                                                              avg_per_class_acc,
                                                                              std_per_class_acc)
        ...
        if test_acc >= best_test_acc:
            ...
            torch.save(model.state_dict(), '%s/model.t7' % file_root)
                # 保存 pred 数组
        np.save('{}/test_pred.npy'.format(file_root), test_pred)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='hgcnn', metavar='N',
                        choices=['pointnet', 'pointnet_norm', 
                                 ...
                                 'hgcnn_cov','graphcnn','graphcnn_norm','graphcnn_cov',
                                 'graphcnn_norm_geo','graphcnn_geo'],
